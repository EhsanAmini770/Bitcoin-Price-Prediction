...
from keras.layers import Dense, SimpleRNN, Input, Activation
...
for train_index, test_index in kf.split(X):
    ...
    rnn_input = Input(shape=input_shape, name='rnn_input')
    inputs = SimpleRNN(150, name='first_layer')(rnn_input)
    ...
    # Train model
    model.fit(X_train, y_train, batch_size=15, epochs=60, shuffle=True, validation_split=0.1, verbose=0)
...
y_test_unscaled = y_sc.inverse_transform(y_test)
y_pred_unscaled = y_sc.inverse_transform(y_pred)
...
future_predictions_unscaled = y_sc.inverse_transform(np.array(future_predictions).reshape(-1, 1))

# Plot future predictions (optional)
plt.figure(figsize=(16, 8))
plt.plot(np.arange(len(y_test_unscaled)), y_test_unscaled, color='black', label='Test Data')
plt.plot(np.arange(len(y_test_unscaled), len(y_test_unscaled) + future_days), future_predictions_unscaled, color='green', label='Future Predictions')
plt.legend()
plt.show()
